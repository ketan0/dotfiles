...
#+begin_src clojure
:yabai-display-mode
[:l [:yabai "window --display prev && /opt/homebrew/bin/yabai -m display --focus prev"]]
[:h [:yabai "window --display next && /opt/homebrew/bin/yabai -m display --focus next"]]
...
[:l [:smarturl "http://localhost:8080"]]
...
[:p [:withsecretsmarturl "$GITHUB_MY_PRS_URL"]]
